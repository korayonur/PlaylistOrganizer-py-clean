...
class PlaylistManagementService: ObservableObject {
    ...
    func createPlaylist(name: String, parentId: UUID? = nil) {
        Task {
            do {
                let newPlaylist = Playlist(name: name, trackCount: 0)
    ...
    func getPlaylistById(_ id: UUID) -> Playlist? {
    ...
    func getChildPlaylists(for parentId: UUID) -> [Playlist] {
